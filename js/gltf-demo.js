...
const GltfModels = {
  ...
  san_miguel: `${GltfRootDir}/SanMiguel/glTF-Binary/SanMiguel.glb`,
};
...
export class GltfDemo extends TinyWebGpuDemo {
  ...
  constructor(rendererClass, startup_model) {
    ...
    if (startup_model in GltfModels) {
      ...
      this.model = GltfModels['san_miguel'];
    }
  ...
  async onInit(device) {
    ...
    await this.onLoadModel(device, this.model);

    console.log(this.gltfRenderer)
    if(this.gltfRenderer){
      // Create the render bundle
      this.bundleEncoder = device.createRenderBundleEncoder({
        colorFormats: ['bgra8unorm'],
        depthStencilFormat: 'depth24plus',
        sampleCount: 4
      });
      this.gltfRenderer.render(this.bundleEncoder);
